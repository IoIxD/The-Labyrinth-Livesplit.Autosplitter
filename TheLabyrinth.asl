...
init {
    vars.GameTitle = "RMCSHNative.dll not found";
    vars.HasPrintedError = false;

    try {
        var module = modules.SingleOrDefault(m => m.ModuleName.Equals("RMCSHNative.dll"));
        ...
        vars.GameTitlePtr = (IntPtr)(vars.FitnessGramPacerTest + 313);

        vars.GameTitle = game.ReadString((IntPtr)vars.GameTitlePtr, 255).Replace("Minecraft Minecraft","Minecraft");
        if(vars.GameTitle == "") {
            vars.GameTitle = "Minecraft";
        }
        ...
        vars.GameTitle = "RMCSHNative.dll not found ("+ex.ToString()+")";
    }
    version = vars.GameTitle;
}
